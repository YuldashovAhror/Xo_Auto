                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        @foreach($comments as $comment)
                            <tr>
                                <th scope="row">{{ $k }}</th>
                                <td><img src="{{ $comment->photo }}" alt="" style="height: 100px; width: 200px"></td>
                                <td>{{ $comment->name }}</td>
                                <td>{{ $comment->star }}</td>
                                <td class="text-center">
                                    ...
                                    <button class="btn btn-xs btn-danger" type="button" data-bs-toggle="modal" data-bs-target="#exampleModalCenter{{ $comment->id }}"><i class="bx bx-trash"></i>Удалить</button>
                                        ...
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    <form action="{{ route('dashboard.commentcompany.destroy', $comment) }}" method="post">
                                                        @csrf
                                                        {{ method_field('delete') }}
